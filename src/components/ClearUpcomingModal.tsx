...
export function ClearUpcomingModal({ calendar, upcomingCount, onClose, onConfirm }: Props) {
      ...
      <div className="modal" style={{ maxWidth: "480px" }}>
        ...
        <div className="modal-body">
          ...
          <div style={{
            ...
            <strong>
              {upcomingCount > 0
                ? `all ${upcomingCount} upcoming event${upcomingCount === 1 ? "" : "s"}`
                : "all upcoming events"}
            </strong>{" "}
            from the <strong>{calLabel} calendar</strong> — including this month, the rest of this year, next year, and all future dates.
            <br /><br />
            This affects every future event regardless of legend, type, or how it was created. There is no undo.
          </div>
        ...
        <div className="modal-foot" style={{ background: "#fef2f2", borderColor: "#fecaca" }}>
          ...
          >
            {loading
              ? "Deleting…"
              : `Delete ${upcomingCount > 0 ? upcomingCount : "all"} upcoming event${upcomingCount === 1 ? "" : "s"}`}
          </button>
        </div>
      </div>
    </div>
